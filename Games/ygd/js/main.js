                ...
                init: function() {
                    ...
                    e.run(function() {
                        ...
                        var n = ~~(o / l.scenes.length),
                            ...
                            sy = 0;
                        if (l.shake > 0) {
                            sx = (Math.random() - .5) * l.shake,
                                sy = (Math.random() - .5) * l.shake,
                                l.shake *= .9,
                                l.shake < .4 && (l.shake = 0)
                        }
                        e.canvas.save().translate(sx, sy);
                        if (!l.died) {
                            l.time = t - l.date;
                            for (var s = 0,
                                    f; f = l.scenes[s]; s++) {
                                ...
                                if (f.hostDied) {
                                    l.died = !0,
                                        ...
                                        f.host.setZoomTransition([1, .85, .7, .55, .4, .25, .1, .03]),
                                        l.shake = 16,
                                        l.flash = .85,
                                        l.shock = {
                                            x: f.host.x,
                            ...
                            if (l.shock) {
                                l.shock.r += 8,
                                    l.shock.a *= .92,
                                    l.shock.a < .05 && (l.shock = null)
                            }
                            t - l.dieDate >= l.dieTimeout && (l.dieDate = null, h())
                        }
                        e.canvas.restore();
                        if (l.shock) {
                            e.canvas.save().globalAlpha(l.shock.a).strokeStyle("#FF3030").lineWidth(4).beginPath().arc(l.shock.x, l.shock.y, l.shock.r, 0, 6.2832).stroke().restore();
                            e.canvas.save().globalAlpha(l.shock.a).strokeStyle("#FFD700").lineWidth(2).beginPath().arc(l.shock.x, l.shock.y, l.shock.r * .6, 0, 6.2832).stroke().restore()
                        }
                        if (l.flash > 0) {
                            e.canvas.save().globalAlpha(l.flash).fillStyle("#FFF").fillScreen().restore(),
                                l.flash *= .88,
                                l.flash < .02 && (l.flash = 0)
                        }
                        var c = a(l.time);
                            ...
                            Scene: e.extend(function(t) {
                                    this.id = t,
                                        ...
                                        this.host = function() {
                                                var sp = [],
                                                    z = [.2, .4, .55, .7, .82, .92, 1];
                                                for (var k = 0; k < t; k++) sp.push(.2);
                                                for (var k = 0; k < z.length; k++) sp.push(z[k]);
                                                return n.get().setStep(2).setZoomTransition(sp)
                                            }(),
                                        this.hostDied = !1,
                                        this.boxes = [],
