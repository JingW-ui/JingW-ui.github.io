...
export class VehicleConfig {
	...
	constructor( config = {} ) {
		...
		this.handling = config.handling || 4.0;
		this.braking = config.braking || 8.0;
...
export class Vehicle {
	...
	constructor( config = {} ) {
		...
		this.config = new VehicleConfig( config );
	...
	update( dt, controlsInput ) {
		...
		if ( controlsInput.touchActive && ( this.inputX !== 0 || this.inputZ !== 0 ) ) {
			...
			this.linearSpeed = THREE.MathUtils.lerp( this.linearSpeed, this.config.maxSpeed, dt * this.config.acceleration );
			...
			const targetAngular = - this.inputX * steeringGrip * this.config.handling * direction;
			this.angularSpeed = THREE.MathUtils.lerp( this.angularSpeed, targetAngular, dt * this.config.handling );
			...
			if ( targetSpeed < 0 && this.linearSpeed > 0.01 ) {
				...
				this.linearSpeed = THREE.MathUtils.lerp( this.linearSpeed, 0.0, dt * this.config.braking );
				...
				this.linearSpeed = THREE.MathUtils.lerp( this.linearSpeed, targetSpeed * this.config.maxSpeed, dt * this.config.acceleration );
