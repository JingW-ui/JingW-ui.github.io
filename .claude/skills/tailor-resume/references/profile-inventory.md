...
## 实习经历
- **网易互娱 · 游戏测试开发工程师（实习）** ｜ 2026.07-2026.09
  - 技术栈：Python · LLM · Agent · Skill 开发 · MCP · Agent/Model 评测
  - 内容：agent + skill 远程 PC 自动化落地（UU 取号 / G66 部署，答辩数据：10 台取号 2h+→10 分钟、100GB 级环境部署 3h→30 分钟）；Agent 评测系统从 0 到 1（三指标选型、「判世界不判自述」判题、L1–L4 题库、多 agent × 多模型 × 多设备并发、评测看板）；MCP 使用与开发（远程设备操控：连接设备、执行终端操作、结果回传）
- **心曜医疗 · 算法工程师实习生** ｜ 2026.03-2026.06
  - 技术栈：Python · LLM · RAG · 向量数据库 · 医疗知识库
  - 内容：**独立开发**医疗领域 RAG 问答系统（架构设计 / 知识库构建 / 检索增强流程优化）；服务器管理系统、多端同步看板等配套工程
...
- 奖学金：国家励志奖学金 ×2、电子科大优秀研究生 ×2、学业奖学金一等奖 ×2、研究生"学术青苗"奖
- 其他：四川省优秀毕业生、绵阳市三好学生
...
- **★ Multi-Agent 协作架构**
- **★ MCP（开发 / 安装 / 使用）**：网易实习期间长期使用与开发远程设备操控 MCP（连接远程设备、执行终端操作、结果回传）
- Prompt 工程
- RAG（检索增强生成）
- Chain-of-Thought
